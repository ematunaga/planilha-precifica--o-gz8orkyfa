...
//   created_at: timestamp with time zone (not null, default: now())
// Table: proposal_conditions
//   id: uuid (not null, default: gen_random_uuid())
//   name: text (not null)
//   content: text (not null)
//   type: text (not null, default: 'distributor'::text)
//   created_at: timestamp with time zone (not null, default: now())
// Table: proposals
//   id: uuid (not null, default: gen_random_uuid())
...
// Table: proposal_conditions
//   PRIMARY KEY proposal_conditions_pkey: PRIMARY KEY (id)
// Table: proposals
//   FOREIGN KEY proposals_created_by_fkey: FOREIGN KEY (created_by) REFERENCES auth.users(id) ON DELETE SET NULL
...
//     USING: ((owner_id = auth.uid()) OR (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = 'Admin'::text))
// Table: proposal_conditions
//   Policy "proposal_conditions_delete" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = 'Admin'::text)
//   Policy "proposal_conditions_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = 'Admin'::text)
//   Policy "proposal_conditions_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
//   Policy "proposal_conditions_update" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (( SELECT profiles.role    FROM profiles   WHERE (profiles.id = auth.uid())) = 'Admin'::text)
// Table: proposals
//   Policy "proposals_delete" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM projects   WHERE ((projects.id = proposals.project_id) AND ((projects.owner_id = auth.uid()) OR (( SELECT profiles.role            FROM profiles           WHERE (profiles.id = auth.uid())) = 'Admin'::text)))))
//   Policy "proposals_insert" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: (EXISTS ( SELECT 1    FROM projects   WHERE ((projects.id = proposals.project_id) AND ((projects.owner_id = auth.uid()) OR (( SELECT profiles.role            FROM profiles           WHERE (profiles.id = auth.uid())) = 'Admin'::text) OR (( SELECT profiles.role            FROM profiles           WHERE (profiles.id = auth.uid())) = 'Editor'::text)))))
//   Policy "proposals_select" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM projects   WHERE ((projects.id = proposals.project_id) AND ((projects.is_public = true) OR (projects.owner_id = auth.uid()) OR (( SELECT profiles.role            FROM profiles           WHERE (profiles.id = auth.uid())) = 'Admin'::text)))))
// Table: user_invitations
